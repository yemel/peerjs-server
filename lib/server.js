...
PeerServer.prototype._initializeHTTP = function() {
  ...
  // Server stats
  this._app.get(this._options.path + 'stats', function(req, res, next) {
    var auth = authParser(req.header('authorization') || ' ');

    if (!auth.username || !auth.password) {
      res.header('WWW-Authenticate', 'Basic realm="PeerJS Server"');
      res.send(401, 'Needs authentication');
      return;
    }

    if (auth.username != self._options.key || auth.password != self._options.auth) {
      res.header('WWW-Authenticate', 'Basic realm="PeerJS Server"');
      res.send(401.2, 'Needs authentication');
      return next();
    }

    var clients = self._clients[self._options.key] || {};
    res.send('Peers connected: ' + Object.keys(clients).length);
    return next();
  });
